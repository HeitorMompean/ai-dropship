...
class TelegramService:
    ...
    async def send_message(
        ...
        target_chat = chat_id if chat_id else self._chat_id
        if not self._token:
    ...
    async def edit_message_text(self, chat_id: int, message_id: int, text: str, reply_markup=None, parse_mode: str = "HTML"):
    ...
    async def answer_callback_query(self, callback_query_id: str, text: str = None):
        if not self._token or not callback_query_id: return {"status": "skipped"}
        url = self._base_url + "/answerCallbackQuery"
